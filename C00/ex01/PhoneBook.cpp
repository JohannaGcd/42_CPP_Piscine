...
void PhoneBook::search_contact(Contact *contact_list, int counter_contact) {
  ...
  if (contact_list != NULL) {
    ...
    std::cout << "Enter the index of the entry you would like to display: "
              << std::endl;
    if (!std::getline(std::cin, char_index) || char_index.empty() ||
        is_blank(char_index)) {
    ...
    if (validate_input(char_index, counter_contact) == true) {
      int int_index = std::stoi(char_index);
      print_column_full("First name: ",
                        contact_list[int_index].get_first_name());
      ...
      print_column_full("Phone number: ",
                        contact_list[int_index].get_phone_number());
      print_column_full("Darkest secret: ",
                        contact_list[int_index].get_darkest_secret());
      std::cout << std::endl;
    } else {
      std::cout
          << "You must enter a number between 0 and (available saved indexes:) "
          << counter_contact - 1 << "." << std::endl;
      return;
    }
  }
}
