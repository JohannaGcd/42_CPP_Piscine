...
static void initMaterias(AMateria **_materias) {
  for (int i = 0; i < 4; i++) _materias[i] = nullptr;
}
...
static void cleanMaterias(AMateria **_materias) {
  for (int i = 0; i < 4; i++) delete _materias[i];
}

MateriaSource::MateriaSource() : IMateriaSource() { initMaterias(_materias); }
...
MateriaSource::~MateriaSource() { cleanMaterias(_materias); }
