...
int main(void) {
  std::cout << "[ TEST = Creating a Bureaucrat with each constructor ]"
            << std::endl;
  ...
  try {
    std::cout
        << "--Trying to instantiate Corinne, with a grade of 0. Should fail--"
        << std::endl;
  ...
  std::cout << "The following test sets A's grade to 0. Should KO."
            << std::endl;
  try {
    A.incrementGrade(50);
  } catch (std::exception& e) {
  ...
  std::cout << std::endl;
  std::cout << "The following test sets A's grade to 151. Should KO."
            << std::endl;
  try {
