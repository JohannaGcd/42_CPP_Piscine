...
Fixed &Fixed::operator=(const Fixed &src) {
  ...
  if (this != &src) this->fixedPoint = src.fixedPoint;
  return *this;
};
...
int Fixed::toInt(void) const { return fixedPoint >> fractionalBits; };
...
Fixed Fixed::operator/(const Fixed &b) const {
  ...
  long long temp =
      ((long long)fixedPoint * (1 << fractionalBits)) / (long long)b.fixedPoint;
  res.setRawBits(temp);
  return res;
};
