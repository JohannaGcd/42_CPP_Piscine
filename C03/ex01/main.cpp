...
static void testAttackAndEnergyConsumption() {
  ...
  a.attack("target-1");
  check(a.getEnergyPoints() == beforeEnergy - 1,
        "ScavTrap attack consumes 1 energy point");
}
...
static void testInheritedBehaviors() {
  ...
  a.beRepaired(3);
  check(a.getHitPoints() == hpBeforeRepair + 3,
        "beRepaired increases hit points");
  check(a.getEnergyPoints() == energyBeforeRepair - 1,
        "beRepaired consumes 1 energy point");
...
static void testCopyConstructorAndAssignment() {
  ...
  check(copied.getHitPoints() == original.getHitPoints(),
        "Copy constructor copies hit points");
  check(copied.getEnergyPoints() == original.getEnergyPoints(),
        "Copy constructor copies energy points");
  check(copied.getAttackDamage() == original.getAttackDamage(),
        "Copy constructor copies attack damage");

  TestScavTrap assigned("Assigned");
  assigned = original;
  check(assigned.getName() == original.getName(), "Assignment copies name");
  check(assigned.getHitPoints() == original.getHitPoints(),
        "Assignment copies hit points");
  check(assigned.getEnergyPoints() == original.getEnergyPoints(),
        "Assignment copies energy points");
  check(assigned.getAttackDamage() == original.getAttackDamage(),
        "Assignment copies attack damage");
}

static void testConstructionDestructionChaining() {
  std::cout << "\n=== testConstructionDestructionChaining ===" << std::endl;
  std::cout << "Expected constructor order: ClapTrap then ScavTrap"
            << std::endl;
  ...
  check(true,
        "Chaining demo executed (verify order in printed "
