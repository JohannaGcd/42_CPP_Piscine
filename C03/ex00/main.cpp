...
static void testExhaustEnergyAndRepair() {
  ...
  check(out.find("does not have enough hit or energy points to perform an "
                 "attack on target.") != std::string::npos,
        "11th attack prints expected failure message");
  check(out.find("does not have enough hit or energy points to be repaired.") !=
            std::string::npos,
...
static void testCopyConstructor() {
  ...
  check(b.getEnergyPoints() == 9,
        "b EP should be 9 after an attack, proving it is an independant object "
        "from a");
}
