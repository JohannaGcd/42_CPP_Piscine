...
class TestDiamondTrap : public DiamondTrap {
 public:
  ...
  TestDiamondTrap(const std::string &name)
      : ClapTrap(name + "_clap_name"), DiamondTrap(name) {}
...
static void testNameSplit() {
  ...
  check(a.getCName() == "Alpha_clap_name",
        "DiamondTrap keeps ClapTrap name with suffixe");
}
...
static void testAttack() {
  ...
  check(out.find("ScavTrap") != std::string::npos,
        "ScavTrap's attack method was called.");
}
...
static void testConstructionDestructionChaining() {
  ...
  std::cout << "Expected constructor order: ClapTrap, FragTrap, ScavTrap, then "
               "DiamondTrap"
            << std::endl;
  std::cout << "Expected destructor order: DiamondTrap then ScavTrap, FragTrap "
               "and ClapTrap"
            << std::endl;
  {
